...
public class AbilityLogic : MonoBehaviour
{
    ...
    public IEnumerator PerformShootCoroutine(LivingEntity caster, LivingEntity victim)
    {
        ...
        if (attackSuccesful)
        {
            CombatLogic.Instance.HandleDamage(CombatLogic.Instance.CalculateDamage(WeaponLogic.Instance.CalculateRandomWeaponDamageValue(caster.myRangedWeapon), victim, caster, shoot.abilityDamageType), caster, victim);
        }
        else
        {
            // TO DO IN FUTURE: method here should be something like CombatLogic.Instance.HandleMiss()
