...
public class Defender : LivingEntity
{
    ...
    public void OnShootButtonClicked()
    {
        ...
        //LevelManager.Instance.HighlightTiles(LevelManager.Instance.GetTilesWithinRangeAndLoS(myRangedWeapon.weaponRange, LevelManager.Instance.Tiles[GridPosition], true));
        LevelManager.Instance.HighlightTiles(LevelManager.Instance.GetTilesWithinRangeAndLoS(myRangedWeapon.weaponRange, TileCurrentlyOn));
    }
    ...
    public void UpdateCurrentAimStatText(int newAimValue)
    {
        myCurrentAimStatText.text = newAimValue.ToString();
    }
    public void UpdateCurrentDefenseStatText(int newDefenseValue)
    {
        myCurrentDefenseStatText.text = newDefenseValue.ToString();
    }
    ...
    public void StartShootProcess(LivingEntity target)
    {
        Ability shoot = mySpellBook.GetAbilityByName("Shoot");
        Debug.Log("Defender.StartShootProcess() called");
        //Enemy enemyTarget = EnemyManager.Instance.selectedEnemy;
        if (IsTargetInRange(target, myRangedWeapon.weaponRange))
        {
            //PerformShoot(enemyTarget, CalculateDamage(shoot.abilityPrimaryValue, enemyTarget, this));
            ClearAllOrders();
            AbilityLogic.Instance.PerformShoot(this, target);
        }
    }
